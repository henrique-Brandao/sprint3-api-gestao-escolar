...
public class ProfessorController : ControllerBase
{
    ...
    public ProfessorController(IProfessorService professorService, AppDbContext context)
    {
    ...
    {
        if (await _context.Professores.AnyAsync(p => p.Email == request.Email))
        {
            return BadRequest(new { mensagem = "Já existe professor com este email." });
        }
    ...
    {
        if (await _context.Professores.AnyAsync(p => p.Id != id && p.Email == request.Email))
        {
            return BadRequest(new { mensagem = "Já existe professor com este email." });
        }
